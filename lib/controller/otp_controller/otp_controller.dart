...
//       }
//       dev.log(
//         '📱 Identifier received: ${identifier.value}',
//         name: 'OtpController',
//       );
//     }
//   }
...
class OtpController extends GetxController {
  var otp = "".obs;
  var isLoading = false.obs; // 🔹 used ONLY by verifyOtp / Continue button
  var isResendLoading =
      false.obs; // 🔹 NEW — used ONLY by resendOtp / Send Again
  var identifier = "".obs;
  var isPhone = false.obs;
  ...
  Future<void> verifyOtp() async {
    ...
    isLoading.value = true; // 🔹 only isLoading — Continue button
    dev.log('🔄 Verifying OTP for: ${identifier.value}', name: 'OtpController');
  ...
  Future<void> resendOtp() async {
    ...
    isResendLoading.value = true; // 🔹 only isResendLoading — Send Again area
    dev.log('🔄 Resending OTP to: ${identifier.value}', name: 'OtpController');

    try {
      final response = await http.post(
        Uri.parse(AppUrl.sendOtp), // 🔹 same endpoint as login send OTP
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
        },
