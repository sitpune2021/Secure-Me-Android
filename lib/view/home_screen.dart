...
class UserHomeScreen extends StatelessWidget {
  ...
  void _onToggleSignal() {
    if (_safetyController.status.value == SignalStatus.inactive) {
      _safetyController.activateEmergency(const LocationModel(
          latitude: 37.7749,
          longitude: -122.4194,
          address: 'San Francisco, CA'));
    } else {
      // Prompt for PIN to cancel (Mock dialog)
      Get.defaultDialog(
        title: "Cancel Emergency?",
        middleText: "Enter Safety PIN (1234)",
        content: Padding(
          padding: const EdgeInsets.symmetric(vertical: 8),
          child: TextField(
            onSubmitted: (val) {
              Get.back();
              _safetyController.cancelEmergency(pin: val);
            },
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      final isActivated = _safetyController.status.value != SignalStatus.inactive;
                ...
                child: Row(
                  ...
                  children: [
                    Column(
                      ...
                      children: [
                        Text('Secure Me', style: GoogleFonts.outfit(fontSize: 24, fontWeight: FontWeight.bold, color: Colors.white)),
                        Text('Personal Safety Dashboard', style: GoogleFonts.outfit(fontSize: 12, color: Colors.white70)),
                      ],
                    ),
                    const CircleAvatar(
                    ...
                    children: [
                      Center(
                        child: Text(
                          ...
                          style: GoogleFonts.outfit(color: Colors.white60, fontSize: 13, fontWeight: FontWeight.normal),
                        ),
                      ),
                      Positioned(
                        left: 4,
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              mainAxisSize: MainAxisSize.min,
              children: [
                Text(title, style: GoogleFonts.outfit(color: Colors.white54, fontSize: 11, fontWeight: FontWeight.w500)),
                Text(content, style: GoogleFonts.outfit(color: Colors.white, fontSize: 13, fontWeight: FontWeight.bold)),
              ],
            ),
          ),
        ],
